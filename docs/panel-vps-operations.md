...
Поддерживается чистая Debian 13. В панели укажите название, IPv4-адрес и `<ПАРОЛЬ_НОВОГО_VPS>`. Логин всегда `root`. Пароль не сохраняется: он находится только в памяти фоновой задачи до её завершения.

Нормальная последовательность этапов: подключение по SSH, проверка Debian 13, установка пакетов, настройка зарубежного туннеля, настройка российского шлюза, handshake, проверка интернета, подключение к балансировке.
